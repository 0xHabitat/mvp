...
library LibHabitatDiamond {
  ...
  event AddressesProviderUpdated(address indexed previousAddressesProvider, address indexed newAddressesProvider);
  ...
  function updateFacet(
    ...
    require(addressesProvider.facetAddressExist(_newFacetAddress), "Addresses provider does not contain the facet.");
    ...
    require(oldFacetAddress != address(0), "Update rule is not followed: first selector does not exist in a diamond.");

    // get old facet selectors from diamond storage
    uint numberOfReplacedSelectors = ds.facetFunctionSelectors[oldFacetAddress].functionSelectors.length;
    ...
    if (numberOfReplacedSelectors < selectors.length) {
      ...
      for (uint i = 0; i < selectors.length; i++) {
        address facetAddress = ds.selectorToFacetAndPosition[selectors[i]].facetAddress;
        require(facetAddress == oldFacetAddress, "Update rule is not followed: tried to update two facets at once.");
      }
    ...
    require(ds.facetFunctionSelectors[oldFacetAddress].functionSelectors.length == 0, "Old facet was not removed.");
  ...
  function removeFacets(address[] memory facets) internal {
    DiamondStorage storage ds = diamondStorage();
    for (uint256 facetIndex; facetIndex < facets.length; facetIndex++) {
      bytes4[] memory functionSelectors = ds.facetFunctionSelectors[facets[facetIndex]].functionSelectors;
