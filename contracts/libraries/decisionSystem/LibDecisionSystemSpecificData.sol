...
library LibDecisionSystemSpecificData {
  function _getMSVotingPowerSpecificData(
    string memory msName
  ) internal view returns (VotingPowerSpecificData memory vpsd) {
  ...
  function _getMSSignersSpecificData(
    string memory msName
  ) internal view returns (SignerSpecificData memory ssd) {
    IManagementSystem.MSData storage msData = LibManagementSystem._getMSDataByName(msName);
  ...
  function _getSecondsProposalExecutionDelayPeriodSigners(
    string memory msName
  ...
  function _getThresholdForInitiatorNumerator(
    string memory msName
  ) internal view returns (uint256) {
    VotingPowerSpecificData memory vpsd = _getMSVotingPowerSpecificData(msName);
  ...
  function _getSecondsProposalExecutionDelayPeriodVP(
    string memory msName
  ) internal view returns (uint256) {
    VotingPowerSpecificData memory vpsd = _getMSVotingPowerSpecificData(msName);
  ...
  function _changedThresholdForInitiatorBytes(
    string memory msName,
    uint256 newThresholdForInitiator
  ) internal view returns (bytes memory newVPSD) {
    VotingPowerSpecificData memory vpsd = LibDecisionSystemSpecificData
      ._getMSVotingPowerSpecificData(msName);
    vpsd.thresholdForInitiator = newThresholdForInitiator;
    newVPSD = abi.encode(vpsd);
  }

  function _changedThresholdForProposalBytes(
    string memory msName,
  ...
  function _changedSecondsProposalVotingPeriodBytes(
    string memory msName,
  ...
  function _changedSecondsProposalExecutionDelayPeriodVPBytes(
    string memory msName,
  ...
  function _changedSecondsProposalExecutionDelayPeriodSignersBytes(
    string memory msName,
    uint256 newSecondsProposalExecutionDelayPeriodSigners
  ) internal view returns (bytes memory newSSD) {
