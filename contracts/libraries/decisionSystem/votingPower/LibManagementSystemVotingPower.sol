...
library LibManagementSystemVotingPower {
  ...
  function _getManagementSystemData(string memory msName) internal pure returns(IManagementSystem.MSData storage msData) {
  ...
  function _getMSVotingPowerSpecificData(string memory msName) internal returns(VotingPowerSpecificData memory vpsd) {
    ...
    vpsd = convertSpecificDataBytesToVPStruct(msData.decisionSpecificData[2]);
  }
  ...
  function _getMSVotingPowerSpecificDataStoragePointer(string memory msName) internal returns(VotingPowerSpecificData storage vpsd) {
    IManagementSystem.MSData storage msData = _getManagementSystemData(msName);
    vpsd = msData.decisionSpecificData[2];
  }

  function _getFreeProposalId(string memory msName) internal view returns(uint256 proposalId) {
    IManagementSystem.MSData storage msData = _getManagementSystemData(msName);
    ...
    proposalId = uint256(msData.proposalsCount);
    msData.activeVotingProposalsIds.push(proposalId);
  }

  function _getMinimumQuorumNumerator(string memory msName) internal view returns(uint64) {
    VotingPowerSpecificData memory vpsd = _getMSVotingPowerSpecificData(msName);
    return vpsd.minimumQuorum;
  }

  function _getThresholdForProposalNumerator(string memory msName) internal view returns(uint64) {
    VotingPowerSpecificData memory vpsd = _getMSVotingPowerSpecificData(msName);
    return vpsd.thresholdForProposal;
  }

  function _getThresholdForInitiatorNumerator(string memory msName) internal view returns(uint64) {
    VotingPowerSpecificData memory vpsd = _getMSVotingPowerSpecificData(msName);
    return vpsd.thresholdForInitiator;
  }

  function _getSecondsProposalVotingPeriod(string memory msName) internal view returns(uint64) {
    VotingPowerSpecificData memory vpsd = _getMSVotingPowerSpecificData(msName);
    return vpsd.secondsProposalVotingPeriod;
  }

  function _getSecondsProposalExecutionDelayPeriod(string memory msName) internal view returns(uint64) {
    VotingPowerSpecificData memory vpsd = _getMSVotingPowerSpecificData(msName);
