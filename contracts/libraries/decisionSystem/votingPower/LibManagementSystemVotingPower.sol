...
library LibManagementSystemVotingPower {
  ...
  function _getManagementSystemData(string memory msName) internal view returns(IManagementSystem.MSData storage msData) {
  ...
  function _getMSVotingPowerSpecificDataStoragePointer(string memory msName) internal view returns(bytes storage vpsd) {
    IManagementSystem.MSData storage msData = _getManagementSystemData(msName);
  ...
  function _getProposal(string memory msName, uint proposalId) internal view returns(IProposal.Proposal storage p) {
    IManagementSystem.MSData storage msData = _getManagementSystemData(msName);
    p = msData.proposals[proposalId];
  }

  function _getActiveVotingProposalsIds(string memory msName) internal view returns(uint256[] storage) {
    IManagementSystem.MSData storage msData = _getManagementSystemData(msName);
