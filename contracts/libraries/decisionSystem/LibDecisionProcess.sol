...
library LibDecisionProcess {
  ...
  event ProposalExecutedSuccessfully(
    string indexed msName,
    uint256 indexed proposalId
  );

  event ProposalExecutedWithRevert(
    string indexed msName,
    uint256 indexed proposalId
  );
  ...
  function createProposal(
    ...
    if (msg.sender != directCaller) {
      // EOA calling
      // is EOA allowed?
      (bool allowed, string memory reason) = decider.isCallerAllowedToCreateProposal(msg.sender, specificData);
      if (!allowed) {
        revert(reason);
      }
