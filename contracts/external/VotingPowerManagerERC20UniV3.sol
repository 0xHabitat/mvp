...
/*
contract StakeContractERC20UniV3WithDelegation is StakeContractERC20UniV3 {

  struct Delegation {
    uint256 amountOfStakedDelegatedERC20GovToken;
    address delegatee;
    uint256 delegatedVotingPower;
    uint256 timeStampToMoveTokens;
    uint256 amountGovTokensToMove;
  }
  ...
  constructor(
    ...
    function stakeGovTokenAndDelegateVotingPower(uint256 _amount, address delegatee) public {
      require(delegatee != address(0), "Wrong delegatee");
      require(_amount > 0, "Nothing to delegate");
      // receive tokens from holder to stake contract
      IERC20(governanceToken).safeTransferFrom(msg.sender, address(this), _amount); // double check
      Delegation storage delegation = delegations[msg.sender];
      // account how much holders tokens are staked and delegated
      delegation.amountOfStakedDelegatedERC20GovToken += _amount;
      ...
      // increase delegatedAmount
      delegation.delegatedVotingPower += _amount;
      // give voting power to delegatee
      votingPowerHolder.increaseVotingPower(delegation.delegatee, _amount);
    }
    // what happen if after undelegateVotingPower call stakeGovTokenAndDelegateVotingPower??
    function undelegateVotingPower() public {
      Delegation storage delegation = delegations[msg.sender];
      require(delegation.delegatee != address(0), "Have not delegate yet.");
      uint delegateeTimeStampToUnstake = votingPowerHolder.getTimestampToUnstake(delegation.delegatee);
      delegation.timeStampToMoveTokens = delegateeTimeStampToUnstake;
      votingPowerHolder.decreaseDelegatedVotingPower(delegation.delegatee, delegation.delegatedVotingPower);
      // remove delegetee
      delegation.delegatee = address(0);
      delegation.delegatedVotingPower = 0;
      // set tokens to move
      delegation.amountGovTokensToMove += delegation.amountOfStakedDelegatedERC20GovToken;
      delegation.amountOfStakedERC20GovToken = 0;
    }

    function moveTokensFromDelegationToStaked() public {
      Delegation storage delegation = delegations[msg.sender];
      require(delegation.amountGovTokensToMove > 0, "Not allow to move");
      uint amountGovTokensToMove = delegation.amountGovTokensToMove;
      delegation.amountGovTokensToMove = 0;
      require(delegation.timeStampToMoveTokens < block.timestamp, "Wait timeStampToMoveTokens");
      // move tokens to staked
      _stakedERC20GovToken[msg.sender] += amountGovTokensToMove;
      // give voting power
      votingPower.increaseVotingPower(msg.sender, amountGovTokensToMove);
    }
}
*/
