...
contract GovernanceFacet {
  ...
  function createGovernanceProposal(
    ...
    if (governanceAction == GovernanceAction.UpdateFacet) {
      ...
      (string memory msName, uint8 decisionType, bytes memory newDecisionData) = abi.decode(callData, (string,uint8,bytes));
      bytes4 changeDecisionData = bytes4(keccak256(bytes("changeDecisionData(string,uint8,bytes)")));
      validCallData = abi.encodeWithSelector(changeDecisionData, msName, decisionType, newDecisionData);
    ...
    } else if (governanceAction == GovernanceAction.ChangeSecondsProposalExecutionDelayPeriodSigners) {
      validCallData = _changeUintInDecisionData(callData, "SecondsProposalExecutionDelayPeriodSigners");
    } else {
      revert("No valid governance action was requested.");
    }
    proposalId = LibDecisionProcess.createProposal("governance", governanceMethods, 0, validCallData);
  }
  ...
  function updateFacetInitProposal(
  ...
  function updateFacetBatchedExecution(
    address newFacetAddress
  ) public  returns(bool result) {
  ...
  function createSeveralGovernanceProposals(
    ...
    for (uint256 i = 0; i < proposalIds.length; i++) {
      proposalIds[i] = createGovernanceProposal(
        governanceActions[i],
        callDatas[i]
      );
    }
  ...
  function decideOnSeveralGovernanceProposals(uint256[] calldata proposalsIds, bool[] calldata decisions)
    external
  ...
  function executeSeveralGovernanceProposals(uint256[] memory proposalIds) external  returns (bool[] memory results) {
    results = new bool[](proposalIds.length);
    for (uint256 i = 0; i < proposalIds.length; i++) {
      results[i] = executeGovernanceProposal(
        proposalIds[i]
      );
    }
  }
  ...
  // helper function
  function _changeUintInDecisionData(bytes memory callData, string memory uintName) internal pure returns(bytes memory validCallData) {
