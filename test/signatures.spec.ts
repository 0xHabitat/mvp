...
import { safeSignTypedData, executeTx, safeSignMessage, calculateSafeTransactionHash, safeApproveHash, buildHabitatTask, logGas, calculateSafeDomainSeparator, preimageSafeTransactionHash, buildSignatureBytes } from "./utils/execution";
...
describe("ManagementSystem", async () => {
    ...
    describe("hashTask", async () => {
        it('should correctly calculate EIP-712 hash', async () => {
            const { safe } = await setupTests()
            const tx = buildHabitatTask({ to: safe.address, nonce: await safe.nonce() })
    ...
    describe("execTask", async () => {
        it('should fail if signature points into static part', async () => {
            ...
            await expect(
                safe.execTask(safe.address, 0, "0x", 0, 0, 0, 0, AddressZero, AddressZero, signatures)
