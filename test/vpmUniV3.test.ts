...
describe('Staking logic', function () {
  ...
  before(async function () {
    ...
    hbtToken = await MockERC20.deploy(
      'Habitat',
      'HBT',
      signer.address,
    ...
    stakeContractERC20UniV3 = await StakeContractERC20UniV3.deploy(
    ...
    nfPositionManager = new ethers.Contract(
      nfPositionManagerAddress,
      nfPositionManagerABI.abi,
      signer
    );
    ...
    if (ethers.BigNumber.from(hbtToken.address).lt(ethers.BigNumber.from(wETHAddress))) {
      ...
      for (const fee in pools) {
        const poolAddress = await nfPositionManager.callStatic.createAndInitializePoolIfNecessary(
          hbtToken.address,
          wETHAddress,
        ...
        await nfPositionManager.createAndInitializePoolIfNecessary(
          hbtToken.address,
          wETHAddress,
          ethers.BigNumber.from(fee),
          priceOneKHBTForETH
  ...
  it('positionValueTest should calculate correct', async function () {
    ...
    for (const fee in pools) {
      ...
      const [tokenId, liquidity, amount0, amount1] = await nfPositionManager.callStatic.mint(
        mintParams
      );
      ...
      const [amount0Principal, amount1Principal] = await positionValueTest.principal(
        tickLower,
        tickUpper,
        liquidity,
        sqrtPriceX96
  ...
  it('Should stake nfPosition correct', async function () {
    ...
    for (let i = 0; i < nftsTester1.inRange.length; i++) {
      // staking in range
      await nfPositionManagerTester1.approve(
        stakeContractTester1.address,
        nftsTester1.inRange[i].tokenId
      );
      ...
      const sqrtPrice = isHBTToken0
        ? await positionValueTest.getSqrtRatioAtTick(
          nftsTester1.inRange[i].tickLower - nftsTester1.inRange[i].tickSpacing
        )
        : await positionValueTest.getSqrtRatioAtTick(
          nftsTester1.inRange[i].tickUpper + nftsTester1.inRange[i].tickSpacing
        );
      const [amount0Principal, amount1Principal] = await positionValueTest.principal(
        nftsTester1.inRange[i].tickLower,
        nftsTester1.inRange[i].tickUpper,
        nftsTester1.inRange[i].liquidity,
        sqrtPrice
      );
      const votingPowerAmountForNFT =
        await stakeContractTester1.getAmountOfVotingPowerForNFTPosition(
          nftsTester1.inRange[i].tokenId
        );
      ...
      if (!isHBTToken0) {
        ...
        await nfPositionManagerTester1.approve(
          stakeContractTester1.address,
          nftsTester1.onlyToken0[i].tokenId
        );
        await expect(
          stakeContractTester1.stakeUniV3NFTPosition(nftsTester1.onlyToken0[i].tokenId)
        ).to.be.revertedWith('Only pair token liquidity is not accepted yet.');

        // here is only hbt liquidity - should stake
        await nfPositionManagerTester1.approve(
          stakeContractTester1.address,
          nftsTester1.onlyToken1[i].tokenId
        );
        ...
        const [amount0Principal, amount1Principal] = await positionValueTest.principal(
          nftsTester1.onlyToken1[i].tickLower,
          nftsTester1.onlyToken1[i].tickUpper,
          nftsTester1.onlyToken1[i].liquidity,
          sqrtPrice
        );
        const votingPowerAmountForNFT =
          await stakeContractTester1.getAmountOfVotingPowerForNFTPosition(
        ...
        await expect(
          stakeContractTester1.stakeUniV3NFTPosition(nftsTester1.onlyToken1[i].tokenId)
        ).to.be.revertedWith('Only pair token liquidity is not accepted yet.');
        ...
        const sqrtPrice = await positionValueTest.getSqrtRatioAtTick(tick);
        const [amount0Principal, amount1Principal] = await positionValueTest.principal(
          nftsTester1.onlyToken0[i].tickLower,
          nftsTester1.onlyToken0[i].tickUpper,
          nftsTester1.onlyToken0[i].liquidity,
          sqrtPrice
        );
        const votingPowerAmountForNFT =
          await stakeContractTester1.getAmountOfVotingPowerForNFTPosition(
...
*/
  async function getNFTs(beneficiar: any) {
    ...
    for (const fee in pools) {
      ...
      const tickUpperR =
        tickLowerClosest + tickSpacing[fee] * (1 + Math.floor(Math.random() * 100));
      // onlyToken0
      ...
      mintParams.fee = fee;
      mintParams.amount0Desired = isHBTToken0
        ? oneKHBT.mul(1 + Math.floor(Math.random() * 10))
        : oneETH.mul(1 + Math.floor(Math.random() * 5));
      mintParams.amount1Desired = isHBTToken0
        ? oneETH.mul(1 + Math.floor(Math.random() * 5))
        : oneKHBT.mul(1 + Math.floor(Math.random() * 10));

      // mint in range
      let nfToken = await mintToken(
        nfPositionManagerNS,
        mintParams,
        tickLowerR,
        tickUpperR,
